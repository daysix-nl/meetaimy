...
function change_berichten_post_type_labels($labels) {
    $new_labels = array(
        'name' => 'Functionaliteiten',
        'singular_name' => 'Functionaliteit',
        'add_new' => 'Nieuwe functionaliteit toevoegen',
        'add_new_item' => 'Voeg nieuwe functionaliteit toe',
        'edit_item' => 'Bewerk functionaliteit',
        'new_item' => 'Nieuwe functionaliteit',
        'view_item' => 'Bekijk functionaliteit',
        'view_items' => '',
        'search_items' => 'Zoek functionaliteiten',
        'not_found' => 'Geen functionaliteiten gevonden',
        'not_found_in_trash' => 'Geen functionaliteiten gevonden in prullenbak',
        'menu_name' => 'Functionaliteiten'
    );

    return $new_labels;
}
add_filter('post_type_labels_post', 'change_berichten_post_type_labels');

function disable_berichten_taxonomies() {
    unregister_taxonomy_for_object_type('category', 'post');
    unregister_taxonomy_for_object_type('post_tag', 'post');
}
add_action('init', 'disable_berichten_taxonomies');
...
//             .toplevel_page_gfb ul li:nth-of-type(1n+12) { display: none !important; }
//             .toplevel_page_gfb ul li:nth-of-type(5) { display: none !important; }
//             .toplevel_page_gfb ul li:nth-of-type(6) { display: none !important; }
//             .toplevel_page_gfb ul li:nth-of-type(7) { display: none !important; }
//             .toplevel_page_gfb ul li:nth-of-type(9) { display: none !important; }
        ";
...
/*
|--------------------------------------------------------------------------
| GEEN EMAIL BIJ AANMAKEN NIEUWE GEBRUIKER
|--------------------------------------------------------------------------
|
| 
| 
...
add_filter('gfb_edit_customer_personal_details_notification', function($flag) {
                            return false;
                        }, 10, 1);

                    add_filter('gfb_add_customer_personal_details_notification', function($flag) {
                        return false;
                    }, 10, 1);
...
function modify_admin_menu_texts() {
    global $menu;

    foreach ($menu as $key => $item) {
        if ($item[0] === 'Gravity Booking') {
            $menu[$key][0] = 'Demo\'s';
        }
    }
}

add_action('admin_menu', 'modify_admin_menu_texts');
...
function custom_frontend_translations($translated_text, $text, $domain) {
    switch ($text) {
            case 'January':
            $translated_text = 'Januari';
            break;
             case 'February':
            $translated_text = 'Februari';
            break;
             case 'March':
            $translated_text = 'Maart';
            break;
             case 'April':
            $translated_text = 'April';
            break;
             case 'May':
            $translated_text = 'Mei';
            break;
             case 'June':
            $translated_text = 'Juni';
            break;
             case 'July':
            $translated_text = 'Juli';
            break;
            case 'August':
            $translated_text = 'Augustus';
            break;
             case 'September':
            $translated_text = 'September';
            break;
            case 'October':
            $translated_text = 'Oktober';
            break;
             case 'November':
            $translated_text = 'November';
            break;
             case 'December':
            $translated_text = 'December';
            break;
            case 'MON':
            $translated_text = 'Maa';
            break;
...
function change_gravity_booking_time_format( $time_format ) {
    // Wijzig de tijdnotatie naar 24 uur zonder AM/PM
    $time_format = 'H:i';

    return $time_format;
}
add_filter( 'gbk_time_format', 'change_gravity_booking_time_format' );
...
add_action('gfb_created_appointment', function( $appointment_id ) {
...
/*
|--------------------------------------------------------------------------
| Meerdere IP adressen voor login pagina
|--------------------------------------------------------------------------
|
| 
| 
...
//     // Controleer of het huidige IP-adres overeenkomt met een toegestaan IP-adres
//     if (!in_array($user_ip, $allowed_ips) && strpos($_SERVER['REQUEST_URI'], '/wp-login.php') !== false) {
